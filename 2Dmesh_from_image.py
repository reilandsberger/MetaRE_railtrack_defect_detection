...
def read_grayscale_bmp(path: str | Path) -> np.ndarray:
    # Read an uncompressed 8-bit grayscale BMP into a top-down NumPy array.
    path = Path(path)
    ...
    width_px = struct.unpack_from("<i", data, 18)[0]
    height_signed = struct.unpack_from("<i", data, 22)[0]
    bit_count = struct.unpack_from("<H", data, 28)[0]
    compression = struct.unpack_from("<I", data, 30)[0]

    if dib_header_size != 40 or bit_count != 8 or compression != 0:
        raise ValueError(
            f"Unsupported BMP format in {path}: "
            f"DIB={dib_header_size}, bpp={bit_count}, compression={compression}"
        )

    width_px = abs(width_px)
    height_px = abs(height_signed)
    top_down = height_signed < 0
    row_stride = ((width_px * bit_count + 31) // 32) * 4

    image = np.empty((height_px, width_px), dtype=np.uint8)
    for row in range(height_px):
        src_row = row if top_down else (height_px - 1 - row)
        start = pixel_offset + src_row * row_stride
        stop = start + width_px
        image[row] = np.frombuffer(data[start:stop], dtype=np.uint8)

    return image


def read_binary_image(path: str | Path, foreground: str = "light") -> np.ndarray:
    # Convert an image file into a boolean mask with configurable foreground polarity.
    path = Path(path)
    if path.suffix.lower() == ".bmp":
        image = read_grayscale_bmp(path)
    else:
...
def load_vertices_from_image_paths(
    paths: Sequence[str | Path],
    foreground: str = "light",
    target_count: int = N_BOUNDARY_VERTICES,
    smooth_window: int = DEFAULT_SMOOTH_WINDOW,
) -> torch.Tensor:
    # Load one or more aligned rail silhouettes; multiple inputs are combined with logical AND.
    if len(paths) == 0:
        raise ValueError("At least one image path is required")

    masks = [read_binary_image(path, foreground=foreground) for path in paths]
    reference_shape = masks[0].shape
    for mask in masks[1:]:
        if mask.shape != reference_shape:
            raise ValueError(f"Image sizes do not match: {reference_shape} vs {mask.shape}")

    combined_mask = np.logical_and.reduce(masks)
    boundary = extract_boundary(combined_mask)
    boundary = resample_closed_curve(boundary, target_count=target_count)
    return normalize_boundary(boundary, smooth_window=smooth_window)
...
def get_dataset_files(dataset_name: str) -> list[Path]:
    # Return the sorted image paths for one defect dataset.
    if dataset_name not in DATASET_DIRS:
        raise ValueError(f"Unknown dataset: {dataset_name}")
    return sorted(DATASET_DIRS[dataset_name].glob("*.bmp"))
...
def load_dataset_vertices(
    ...
    # Convert one or more aligned defect datasets into boundary loops and the upper rail segment used in simulation.
    ...
    if len(dataset_names) == 1:
        ...
        file_groups = list(zip(*(files[:num_samples] for files in file_lists)))

    vertices_list = [
        load_vertices_from_image_paths(
            paths=[file_group] if isinstance(file_group, Path) else list(file_group),
            foreground="light",
...
def generate_dataset_fields(
    ...
    for start in tqdm(
        ...
        psi_rail_batch, psi_ms_batch = field(vertices_batch[start:stop])
        psi_rail_list.append(psi_rail_batch)
        psi_ms_list.append(psi_ms_batch)
...
for dataset_name in DATASET_DIRS:
    ...
    sample = load_vertices_from_image(
        sample_path,
        foreground="light",
        target_count=N_BOUNDARY_VERTICES,
