...
range_defect = torch.load("range_dist_5mm.pt")
depth_defect = torch.load("depth_dist_5mm.pt")
...
psi_ms = torch.load("psi_ms_dist_5mm.pt").to(torch.complex64)
psi_ms = psi_ms / torch.sqrt(torch.mean(psi_ms.abs()**2))
psi_ms0 = torch.load("psi_ms_nodefect_5mm.pt").to(torch.complex64)
