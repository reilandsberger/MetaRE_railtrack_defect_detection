...
range_defect = torch.load(RAILDEFECT_DIR / "range_dist_5mm.pt")
depth_defect = torch.load(RAILDEFECT_DIR / "depth_dist_5mm.pt")
...
psi_ms = torch.load(RAILDEFECT_DIR / "psi_ms_dist_5mm.pt").to(torch.complex64)
psi_ms = psi_ms / torch.sqrt(torch.mean(psi_ms.abs()**2))
psi_ms0 = torch.load(RAILDEFECT_DIR / "psi_ms_nodefect_5mm.pt").to(torch.complex64)
...
ax.hist(test_l2_gap_init.numpy() * 100, bins=np.linspace(0,500,101),  alpha=0.75, label=r"Initial")
ax.hist(test_l2_gap.numpy() * 100, bins=np.linspace(0,500,101), alpha=0.75, label=r"After training")
...
ax[0].set(xticks=np.linspace(0,1.2,7),yticks=np.linspace(0,1.2,7))
...
ax[1].set(xticks=np.linspace(0,6,7),yticks=np.linspace(0,6,7))

x = np.arange(-W/2 + W/500, W/2, W/250) / mm
